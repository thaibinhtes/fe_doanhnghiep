<template>
  <Modal v-if="open" @close="emit('close')">
    <template #body>
      <div class="no-scrollbar relative z-1 mx-auto max-h-[min(90vh,100dvh)] w-full max-w-5xl overflow-y-auto rounded-2xl bg-white p-4 dark:bg-gray-900 sm:p-6 lg:p-8">
        <div class="mb-5 flex items-start justify-between gap-4">
          ...
          <button type="button" class="rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 dark:hover:bg-gray-800" @click="emit('close')">
...
async function loadJobs() {
  ...
  try {
    const response = await cooperativeService.getImportJobs(1, 30)
...
async function selectJob(jobId: number) {
  ...
  try {
    ...
    selectedJob.value = { ...detail, summary: detail.summary ?? { imported: detail.result?.imported ?? 0, duplicates: detail.result?.duplicates ?? detail.result?.updated ?? 0, failed: detail.result?.failed ?? 0 } }
    await loadRows(1)
  } catch {
