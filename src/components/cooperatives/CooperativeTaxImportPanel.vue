<template>
  <div class="space-y-4">
    <div v-if="showHeader" class="flex items-start justify-between gap-4">
      <div>
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white sm:text-xl">
          Import hợp tác xã đóng thuế
        </h2>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Upload file Excel để cập nhật danh sách hợp tác xã đóng thuế. Import chạy nền — bạn sẽ nhận thông báo khi hoàn tất.
        </p>
      </div>
      <button
        v-if="showClose"
    ...
    <ImportLoadingSkeleton
      v-if="dispatching"
      title="Đang khởi tạo import hợp tác xã đóng thuế..."
...
async function loadColumnMapDefaults() {
  try {
    const config = await taxManagementService.getCooperativeTaxImportColumnMap()
...
async function handleImport() {
  ...
  try {
    const dispatchResult = await taxManagementService.importCooperativeTaxFromExcel(selectedFile.value, {
